...
//RoAR mod COMMAND LINES BEGIN
		...
		if(( Q_stricmp( arg1, "origin" ) == 0 ) || (Q_stricmp(arg1, "amorigin") == 0)){
  ...
  int i;
   ...
   for(i = 0; i < level.maxclients; i++) { 
      if(g_entities[i].client->pers.connected == CON_CONNECTED) { 
		 ...
		 if (!g_entities[i].client->pers.admin && !g_entities[i].client->ojpClientPlugIn && !(g_entities[i].r.svFlags & SVF_BOT)){
			 trap_SendServerCommand(ent-g_entities, va("print \"\n\""));
		 }
		 if (g_entities[i].client->pers.admin){
	...
	*/
	...
	else if (Q_stricmp(cmd, "debugShipDamage") == 0)
	{
      ...
      { 
		 ...
		 if (ent->client->pers.admin == qtrue || recipient->client->pers.admin == qtrue)
		 {
			 // Not logging them in once more.
			 return;
		 }
		 ...
		 if ( !Q_stricmp( pass, g_adminPassword.string ) ) {
		 ent->client->pers.admin = qtrue;
         trap_SendServerCommand( -1, va("print \"%s has signed in as an on-duty admin.\n\"", ent->client->pers.netname));
         return;
		 }
		 else {
